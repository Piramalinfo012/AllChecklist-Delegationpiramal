...
const CONFIG = {
  ...
  DRIVE_FOLDER_ID: "1GrG_LuUI9387mah0e9mg4wJS-bk5qzHy",
...
function DelegationDataPage() {
  ...
  const fetchSheetData = useCallback(async () => {
    try {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        // Add Priority column from column U (index 20)
        rowData["col20"] = rowValues[20] || ""
  ...
  const handleSelectItem = useCallback((id, isChecked) => {
    // Only allow admin to select items
    if (!isAdmin) return
  ...
  const handleCheckboxClick = useCallback(
    (e, id) => {
      ...
      // Only allow admin to interact with checkboxes
      if (!isAdmin) {
        e.preventDefault()
        return
      }
  ...
  const handleSelectAllItems = useCallback(
    (e) => {
      ...
      // Only allow admin to select all
      if (!isAdmin) {
        e.preventDefault()
        return
      }
  ...
  const handleNextTargetDateChange = useCallback((id, value) => {
    // Only allow admin to change target date
    if (!isAdmin) return
  ...
  const handleSubmit = async () => {
    ...
    if (!isAdmin) {
      alert("Only administrators can submit task completions.")
      return
    }
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
          ...
          <div className="flex space-x-4">
            ...
            {!showHistory && (
              <button
                ...
                disabled={selectedItemsCount === 0 || isSubmitting || !isAdmin}
                className={`rounded-md py-2 px-4 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed ${
                  isAdmin 
                    ? "bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700" 
                    : "bg-gray-400 cursor-not-allowed"
                }`}
                title={!isAdmin ? "Only administrators can submit task completions" : ""}
              >
        ...
        {/* Admin-only notification for non-admin users */}
        {!isAdmin && !showHistory && (
          <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-md">
            <p className="text-sm">
              <strong>Note:</strong> Only administrators can mark tasks as complete. You can view your assigned tasks here, but task completion must be done by an admin.
            </p>
          </div>
        )}
                  ...
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      <input
                        ...
                        disabled={!isAdmin}
                        title={!isAdmin ? "Only administrators can mark tasks as complete" : "Select all tasks"}
                      />
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      Task ID
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider bg-cyan-50">
                      Priority
                    </th>
                    <th
                        ...
                        >
                          <td className="px-6 py-4 whitespace-nowrap">
                            <input
                              ...
                              disabled={!isAdmin}
                              title={!isAdmin ? "Only administrators can mark tasks as complete" : "Mark task as complete"}
                            />
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap bg-cyan-50">
                          ...
                          <td className={`px-6 py-4 whitespace-nowrap ${!account["col17"] ? "bg-blue-50" : ""}`}>
                            <select
                              ...
                              title={!isAdmin ? "Only administrators can change task status" : ""}
                            >
                          ...
                          <td className={`px-6 py-4 whitespace-nowrap ${!account["col17"] ? "bg-indigo-50" : ""}`}>
                            <input
                              type="date"
                              disabled={!isSelected || statusData[account._id] !== "Extend date" || !isAdmin}
                              value={
                              ...
                              title={!isAdmin ? "Only administrators can set target dates" : ""}
                            />
                          </td>
                          <td className={`px-6 py-4 whitespace-nowrap ${!account["col17"] ? "bg-purple-50" : ""}`}>
                            <input
                              ...
                              title={!isAdmin ? "Only administrators can add remarks" : ""}
                            />
                          </td>
                              ...
                              <label
                                className={`flex items-center ${
                                  ...
                                  account["col9"]?.toUpperCase() === "YES"
                                    ...
                                    : "text-purple-600"
                                } ${isAdmin ? "hover:text-purple-800" : ""}`}
                                title={!isAdmin ? "Only administrators can upload images" : ""}
                              >
