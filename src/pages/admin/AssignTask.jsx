...
export default function AssignTask() {
  ...
  const handleChange = (e) => {
    ...
    // Clear completion date and time when frequency changes from one-time to recurring
    if (name === "frequency" && value !== "one-time") {
  ...
  const fetchMasterSheetOptions = async () => {
    try {
      const masterSheetId = "1d7cv-LcuvL5ckeVyXreqB--E2Hp1_HRadzXjiUnIAKs";
  ...
  const getLastTaskId = async (sheetName) => {
    try {
      const url = `https://docs.google.com/spreadsheets/d/1d7cv-LcuvL5ckeVyXreqB--E2Hp1_HRadzXjiUnIAKs/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        sheetName
      )}`;
  ...
  // Updated fetchWorkingDays function
const fetchWorkingDays = async (department) => {
  try {
    const sheetId = "1d7cv-LcuvL5ckeVyXreqB--E2Hp1_HRadzXjiUnIAKs";
    ...
    // Determine which column to use based on department
    let columnIndex;
    if (department === "Plant") {
      columnIndex = 0; // Column A for Plant Working Dates
    } else if (department === "Office") {
      columnIndex = 4; // Column E for Office Working Dates
    } else {
      // For other departments, use the general working days (Column A)
      columnIndex = 0;
    }

    // Extract dates from the appropriate column
    const workingDays = [];
    data.table.rows.forEach((row) => {
      if (row.c && row.c[columnIndex] && row.c[columnIndex].v) {
        let dateValue = row.c[columnIndex].v;
    ...
    console.log(`Fetched ${workingDays.length} working days for ${department}`);
    return workingDays;
  } catch (error) {
...
// Updated generateTasks function with department-specific working days validation
const generateTasks = async () => {
  if (!date || !formData.doer || !formData.description || !formData.frequency || !formData.department) {
    alert("Please fill in all required fields including department.");
    return;
  }

  // Fetch working days based on selected department
  const workingDays = await fetchWorkingDays(formData.department);
  
  // Check if working days are available for the selected department
  if (workingDays.length === 0) {
    alert(`No working days found for ${formData.department} department. Please check the Working Day Calendar sheet or contact administrator.`);
    return;
  }
  ...
  // If no future working days are available from the selected date for this department
  if (futureDates.length === 0) {
    alert(`No working days found for ${formData.department} department on or after your selected date. Please choose a different start date or contact administrator to update the Working Day Calendar.`);
    return;
  }
  ...
  if (startIndex === -1) {
    ...
    alert(`The selected date (${startDateStr}) is not a working day for ${formData.department} department. The next available working day will be used instead: ${futureDates[0]}`);
  }
  ...
  if (formData.frequency === "one-time") {
    ...
    tasks.push({
      ...
      completionDate: completionDate ? formatDateToDDMMYYYY(completionDate) : "",
    ...
    while (currentIndex < futureDates.length) {
      ...
      tasks.push({
        ...
        requireAttachment: formData.requireAttachment,
        priority: priority, // Add priority to task object
        completionDate: "", // No completion date for recurring tasks
        completionTime: "",
      });
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    // First verify all dates against the working day calendar
    const department = formData.department;
    const workingDays = await fetchWorkingDays(department);
    
    // Check each task's date against the working days
    const allDatesValid = generatedTasks.every(task => {
      return workingDays.includes(task.dueDate);
    });

    if (!allDatesValid) {
      alert(`One or more selected dates are not working days for ${department} department. Please adjust your dates.`);
      setIsSubmitting(false);
      return;
    }

    // Determine sheet based on department selection
   // Determine sheet based on frequency - one-time always goes to DELEGATION
const submitSheetName = formData.frequency === "one-time" ? "DELEGATION" : 
    ...
    // Prepare tasks data - Include completion date and time in the data mapping
    // Prepare tasks data - Include completion date and time in the data mapping
const tasksData = generatedTasks.map((task, index) => {
  const baseData = {
    ...
    startDate: `${task.dueDate} ${time}`, // Combine date and time
    freq: task.frequency,
    enableReminders: task.enableReminders ? "Yes" : "No",
    requireAttachment: task.requireAttachment ? "Yes" : "No"
  };

  // For DELEGATION sheet only, add priority and completion date
  if (submitSheetName === "DELEGATION") {
    baseData.priority = task.priority || priority; // Column U
    baseData.completionDate = task.completionDate && task.completionTime ? `${task.completionDate} ${task.completionTime}` : ""; // Column T
  } else {
    // For Plant and Office sheets, add priority
    baseData.priority = task.priority || priority;
  }
  ...
  return baseData;
    ...
    console.log("Submitting data:", tasksData); // Debug log

    await fetch(
      "https://script.google.com/macros/s/AKfycbyo7fg3XgjpAHjztPI1q_01tR6DN69EZN-gfK81Bq2ae5EPADuR5lrhy6WMjRhavjoVng/exec",
      {
        method: "POST",
        body: formPayload,
        mode: "no-cors",
...
<div className="grid gap-4 md:grid-cols-2">
  <div className="space-y-2 relative"> {/* Add relative positioning here */}
    ...
    <div className="relative">
      ...
      {showCalendar && (
        <div className="absolute z-50 mt-1"> {/* Increase z-index to 50 */}
          <CalendarComponent
            date={date}
                            ...
                            {generatedTasks.slice(0, 20).map((task, index) => (
                              ...
                              >
                                ...
                                <div className="text-xs text-purple-600">
                                  ...
                                  {task.completionDate && (
                                    <span> | Completion: {task.completionDate} {task.completionTime}</span>
                                  )}
                                </div>
