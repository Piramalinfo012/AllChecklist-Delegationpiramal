...
export default function AdminDashboard() {
  ...
  // NEW: State for user department mapping (same as AdminLayout)
  ...
  // NEW: Function to fetch user department mapping (same as AdminLayout)
  const fetchUserDepartments = async () => {
    try {
      setIsLoadingDepartments(true)
      
      // Call your Google Apps Script to fetch the Whatsapp sheet data
      const response = await fetch(`${APPS_SCRIPT_URL}?sheet=Whatsapp&action=fetch`)
  ...
  // NEW: Function to get accessible departments based on user role (same logic as AdminLayout)
  const getAccessibleDepartments = () => {
    ...
    if (role === "admin") {
      return ["Select Department", "Plant", "Office"]
    }
    ...
    // Get department from the mapping
    ...
    if (!actualDepartment && isLoadingDepartments) {
      const fallbackMap = {
        'admin': 'Plant',
        'rahul sir': 'Plant',
        'subir roy': 'Plant',
    ...
    if (!actualDepartment) {
      return ["Select Department"] // Show only select option if no department found
    }
    
    // Return user's specific department
    return ["Select Department", actualDepartment]
  }
  ...
  const fetchMasterSheetColumnA = async () => {
    try {
      ...
      if (data?.table?.rows) {
        ...
        // NEW: Apply role-based filtering to master sheet options
        const accessibleDepartments = getAccessibleDepartments()
        const filteredOptions = columnAValues.filter(value => {
          // If user is admin, show all options
          if (sessionStorage.getItem('role') === 'admin') {
            return true
          }
          // For regular users, only show their department
          return accessibleDepartments.includes(value)
        })

        const options = accessibleDepartments // Use role-based departments instead of all options
      ...
      const response = await fetch(
        `https://docs.google.com/spreadsheets/d/1d7cv-LcuvL5ckeVyXreqB--E2Hp1_HRadzXjiUnIAKs/gviz/tq?tqx=out:json&sheet=MASTER`
      )
      ...
      // NEW: Apply role-based filtering to fallback data too
      const accessibleDepartments = getAccessibleDepartments()
      const options = accessibleDepartments
      setMasterSheetOptions(options)
  ...
  // NEW: useEffect to fetch user departments on mount
  useEffect(() => {
  ...
  // NEW: useEffect to update master sheet options when user department mapping changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Remove the fetchMasterSheetColumnA() call from here since it's now called when userDepartmentMap updates
  }, [])
      ...
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Department dropdown - NEW: Now shows role-based options */}
            <select
        ...
        <div className="w-full overflow-hidden rounded-lg border border-gray-200 bg-white">
          <div className="grid grid-cols-3">
            ...
            >
              Upcoming Tasks
              <span className="block text-xs mt-1">
                {dashboardType === "checklist" ? "Tomorrow's Tasks" : "Future Tasks"}
              </span>
            </button>
            <button
