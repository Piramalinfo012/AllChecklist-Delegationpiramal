...
const CONFIG = {
  ...
  DRIVE_FOLDER_ID: "18hNxAL2Oy5ouDCOkxOE7C2zUidPxpnBX",
  ...
  PAGE_CONFIG: {
    title: "Office Tasks",
    historyTitle: "Office Task History",
...
function AccountDataPage() {
  ...
  const getPriorityWeight = (priority) => {
    if (!priority) return 999 // No priority goes to the end
    const priorityStr = priority.toString().toLowerCase()
    if (priorityStr.includes('high') || priorityStr.includes('urgent')) return 1
    if (priorityStr.includes('medium') || priorityStr.includes('normal')) return 2
    if (priorityStr.includes('low')) return 3
    return 4 // Unknown priority types
  }

  const sortDateWise = (a, b) => {
    // First sort by priority
    const priorityA = getPriorityWeight(a["col15"])
    const priorityB = getPriorityWeight(b["col15"])
  ...
  const getPriorityBadge = (priority) => {
    if (!priority) return { className: "bg-gray-100 text-gray-800", text: "—" }
    
    const priorityStr = priority.toString().toLowerCase()
    if (priorityStr.includes('high') || priorityStr.includes('urgent')) {
      return { className: "bg-red-100 text-red-800 font-semibold", text: priority }
    }
    if (priorityStr.includes('medium') || priorityStr.includes('normal')) {
      return { className: "bg-yellow-100 text-yellow-800", text: priority }
    }
    if (priorityStr.includes('low')) {
      return { className: "bg-green-100 text-green-800", text: priority }
    }
    return { className: "bg-blue-100 text-blue-800", text: priority }
  }
    ...
    try {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        const columnHeaders = [
          ...
          { id: "col15", label: "Priority", type: "string" },
        ]
        ...
        console.log(`Row ${rowIndex}: Task ID = ${rowData.col1}, Priority = ${rowData.col15}, Google Sheets Row = ${googleSheetsRowIndex}`)
          ...
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 border-b border-purple-100 p-4">
            ...
            <p className="text-purple-600 text-sm">
              {showHistory
                ...
                : CONFIG.PAGE_CONFIG.description} {!showHistory && "(Sorted by Priority, then Date)"}
            </p>
          </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredHistoryData.length > 0 ? (
                      filteredHistoryData.map((history) => {
                        const priorityBadge = getPriorityBadge(history["col15"])
                        return (
                          <tr key={history._id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap bg-orange-50">
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${priorityBadge.className}`}>
                                {priorityBadge.text}
                              </span>
                            </td>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredAccountData.length > 0 ? (
                    filteredAccountData.map((account) => {
                      ...
                      const priorityBadge = getPriorityBadge(account["col15"])
                      return (
                        <tr key={account._id} className={`${isSelected ? "bg-purple-50" : ""} hover:bg-gray-50`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap bg-orange-50">
                            <input
                              ...
                              className="border rounded-md px-2 py-1 w-full border-gray-300"
                            />
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap bg-green-50">
                            {account.image ? (
